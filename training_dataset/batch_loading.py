...
def make_dataset(
    ...
    """
    Creates a dataset from processed commands and contexts, and saves it as a parquet file.

    Args:
        processed_commands_path (Path): Path to the .jsonl file containing the processed commands.
        contexts (Dict[str, str]): A dictionary mapping context IDs to context strings.
        save_path (Path): The file path where the dataset will be saved.
        dataset_name (str): The name of the dataset for error handling purposes.
...
def load_contexts(dataset_name: str) -> Dict[str, str]:
    """
    Loads contexts from a JSON file and returns them as a dictionary.

    Args:
        dataset_name (str): The name of the dataset used to identify the correct file.

    Returns:
        Dict[str, str]: A dictionary where the keys are context IDs and the values are context sentences.
    """
