...
def load_contexts(dataset_name: str) -> Dict[int, str]:
    filename = Path(f"datasets/contexts_{dataset_name}_4o.json")
...
if __name__ == "__main__":
    ...
    dataset_names = ['wiki', 'news', 'literature']
    for dataset_name in dataset_names:
        file_path = f"commands/number_{dataset_name}_4o.txt"
        ...
        if batch_job.status != "completed":
            continue
        print(batch_job)
        result_file_name = f"commands/results_{dataset_name}_4o.jsonl"
        result_file_id = batch_job.output_file_id
        result = client.files.content(result_file_id).content
        with open(result_file_name, "wb") as file:
            file.write(result)

        final_save_path = Path(f"datasets/{dataset_name}_4o.parquet")
