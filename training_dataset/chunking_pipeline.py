...
def save_jobs(
    ...
    Args:
        sentences (List[Dict[str, str]]): A list of dictionaries, where each dictionary contains an 'id' and a 'combined_sentence' to be processed.
        filename (Path): The file path where the jobs will be saved.
        model (str, optional): The model to be used for processing the sentences. Defaults to "text-embedding-3-small".
...
def _make_embedding_dict(file_path):
    """
    Reads a JSONL file and constructs a dictionary mapping each ID to its corresponding embedding.

    Args:
        file_path (str or Path): The path to the JSONL file containing the embeddings.

    Returns:
        Dict[str, List[float]]: A dictionary where each key is an ID and the value is the corresponding embedding list. If an embedding cannot be retrieved, the ID is mapped to a list of zeros with a length of 1536.
    """
    returned_dict = {}
...
def save_combined_embeddings_jobs(sentences: List[Dict[str, str]]) -> None:
    """
    Saves combined embedding jobs to a JSONL file and processes the API request for embeddings.

    Args:
        sentences (List[Dict[str, str]]): A list of dictionaries containing sentences to be saved as jobs.

    Returns:
        Dict[str, List[float]]: A dictionary mapping each ID to its corresponding embedding retrieved from the API.
    """
...
def remove_start_dot(chunks: List[str]):
    """
    Removes a leading dot from each string in the provided list of chunks.

    Args:
        chunks (List[str]): A list of strings (chunks) from which to remove leading dots.

    Returns:
        List[str]: A new list of strings with leading dots removed and any surrounding whitespace trimmed.
    """
    updated_chunks = []
    for chunk in chunks:
        if chunk.startswith("."):
