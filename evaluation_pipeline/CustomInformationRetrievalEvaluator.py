...
class InformationRetrievalEvaluator(SentenceEvaluator):
    ...
    def __call__(
            # Dodato
        self, model: "SentenceTransformer", output_path: str = None, openAI_model: str = None, epoch: int = -1, steps: int = -1, *args, **kwargs
    ) -> Dict[str, float]:
        if epoch != -1:
            if steps == -1:
        ...
        scores = self.compute_metrices(model, openAI_model=openAI_model, *args, **kwargs)
    ...
    def get_embedding(self, text, model="text-embedding-3-small", dimension=None):
        text = text.replace("\n", " ")

        embedded_text = self.client.embeddings.create(input = [text], model=model).data[0].embedding
        if not dimension:
